...
async function repackAssetsLoader(assetData) {
  ...
  try {
    ...
    const largestVariantSize = Math.max(...assets.map(asset => asset.data.length));
    const shouldInlineAsset = !!options.inline && (!options.maxInlineSize || largestVariantSize <= options.maxInlineSize);
    let result;
