...
exports.Terminal = exports.MultiPlatformTerminal = void 0;
...
const moveCursor = _nodeUtil.default.promisify(_nodeReadline.default.moveCursor);
const clearScreenDown = _nodeUtil.default.promisify(_nodeReadline.default.clearScreenDown);
/**
...
function getTTYStream(stream) {
  if (stream instanceof _nodeTty.default.WriteStream && stream.isTTY && stream.columns >= 1) {
    return stream;
  }
  return null;
}
...
class Terminal {
  ...
  constructor(stream, {
    ...
    this._rawStreamWrite = stream.write.bind(stream);
    ...
    this._writeStatusThrottled = (0, _throttleit.default)(status => {
      this._writeRaw(status);
    }, 3500);
    this._patchTTYStreamWrites();
  }
  _patchTTYStreamWrites() {
    if (!this._ttyStream) {
      return;
    }
    ...
    this._stream.write = (chunk, encodingOrCallback, maybeCallback) => {
      const encoding = typeof encodingOrCallback === 'string' ? encodingOrCallback : undefined;
      const callback = typeof encodingOrCallback === 'function' ? encodingOrCallback : maybeCallback;
      const shouldCaptureExternalWrite = !this._isInternalWrite && this._hasVisibleStatus();
  ...
  async _writeInternal(chunk, encoding) {
  ...
  async _clearCurrentStatus(ttyStream, statusStr) {
  ...
  async _update() {
    ...
    this._logLines = [];
    this._externalWrites = [];
    if (statusStr === nextStatusStr && logLines.length === 0 && externalWrites.length === 0) {
      return;
    }
    ...
    if (ttyStream) {
      ...
      this._writeStatusThrottled(nextStatusStr.length > 0 ? nextStatusStr + '\n' : '');
    }
  }
  ...
  status(format, ...args) {
    ...
    const statusStr = _nodeUtil.default.format(format, ...args);
    this._nextStatusStr = this._ttyStream ? chunkString(statusStr, this._ttyStream.columns).join('\n') : statusStr;
    this._scheduleUpdate();
   ...
   */
  log(format, ...args) {
    this._logLines.push(_nodeUtil.default.format(format, ...args));
    this._scheduleUpdate();
  }
...
class MultiPlatformTerminal extends Terminal {
  ...
  status(platform, ...args) {
    if (this.checkAllPlatformsDone()) {
      this.persistStatus();
      this.platformStatuses.clear();
      return '';
    }
    this.platformStatuses.set(platform, _nodeUtil.default.format(...args));
  ...
  checkAllPlatformsDone() {
    const statuses = [...this.platformStatuses.values()];
    return Boolean(statuses.length && statuses.every(status => COMPILED_REGEX.test(status)));
  }
}
exports.MultiPlatformTerminal = MultiPlatformTerminal;